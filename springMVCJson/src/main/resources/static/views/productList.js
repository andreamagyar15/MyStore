window.addEventListener("load",function(){
  ...
  function validateAndSendForm(event){
    ...
    var type="";
	var fileUp=document.getElementById("file-upload");
    switch (element) {
      case 0:
        type="tshirts";
        break;
    ...
    if (!ok){
	 ...
	 // var r = new FileReader();
		//r.onload = function(){
			...
			//	return r.result;
			//}
			//newProduct.uploadImage(fileimg.name.split(".")[0],r.result,cookieFromStorage)
			
		//};
		//r.readAsDataURL(fileimg);
		readImage(fileimg)
		.then(function(responseImage){
		object.imageName=fileimg.name.split(".")[0];
		object.imageInput=responseImage;
			...
			contentRootUrl="http://localhost:8090/products";
				...
				.then(function(response){
					console.log(response);
					displayContent(contentRootUrl,response);
					var formdata=document.getElementById("newProductForm");
					formdata.reset();
...
function displayElements(content){
  ...
  var template='<div class="col-md-3 col-sm-6 portfolio-item" id='+content.id+'>';
  ...
  template+='<img class="img-fluid" src="'+convertImage(content.image)+'" alt="">';
  template+='<br>';
...
function convertImage(imageString){
